...
void compileVarDecls(void) {
  ...
  if (lookAhead->tokenType == KW_VAR) {
    ...
    do {
      ...
      node = (ObjectNode*)malloc(sizeof(ObjectNode));
      node->object = varObj;
      node->next = NULL;
	...
	node->object->varAttrs->type = tmpType;
	declareObject(node->object);
	node = node->next;
      }
...
Type* compileLValue(void) {
  ...
  case OBJ_PARAMETER:
    ...
    } else if(var->paramAttrs->kind == PARAM_REFERENCE){
...
void compileAssignSt(void) {
  ...
  if(lookAhead->tokenType == SB_EQ || lookAhead->tokenType == SB_NEQ ||
    ...
    checkTypeEquality(expType, expType2);
    ...
    eat(SB_QUESTION);
    expType3 = compileExpression();
    checkTypeEquality(varType,expType3);
    genST();
    eat(SB_COLON);
    updateFJ(fjInstruction,getCurrentCodeAddress());
    expType4 = compileExpression();
    checkTypeEquality(varType,expType4);
    genST();
  } else {
    checkTypeEquality(varType, expType);
    genST();   
